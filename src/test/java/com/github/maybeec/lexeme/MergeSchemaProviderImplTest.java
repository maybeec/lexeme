...
public class MergeSchemaProviderImplTest {
  ...
  /**
   * Tests if an MergeSchema with an empty namespace is returned<br/>
   * Folder: src/test/resources/provider<br/>
   * Retrieve: namespace_empty.xml
   */
  @Test
  public void testGetEmptyNamespace() {

    MergeSchemaProvider provider = MergeSchemaProviderImpl.getProviderForPath(this.path);
    MergeSchema schema = provider.getMergeSchemaForNamespaceURI("");
    assertEquals("Wrong merge schema returned", "", schema.getDefinition().getNamespace());
