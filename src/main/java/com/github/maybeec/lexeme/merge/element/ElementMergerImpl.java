...
public class ElementMergerImpl implements ElementMerger {
  ...
  public Element merge(Element element1, Element element2, ConflictHandlingType conflictHandling)
    ...
    // If the handling indicates that the elements are leaf elements (i.e. their subtrees musn't been merged) just
    // return a clone of the input elements, according to the chosen ConflictHandling
    if (this.handling.isIsLeaf() && conflictHandling.isBasePrefering()) {
      return element1.clone();
    }
    if (this.handling.isIsLeaf() && !conflictHandling.isBasePrefering()) {
      return element2.clone();
    }
  ...
  private Handling getHandlingFromScopeForElement(Element element) throws ElementsCantBeMergedException {
    ...
    try {
      ...
      String message =
          String.format("%s: Caught unexpected XPathExpressionException. Probaply the 'where'-statement is corrupt:%s",
              this.logId, e.getMessage());
  ...
  List<Handling> getScopeFromRef(String scopeRef, String namespaceRef) {
    ...
    List<Handling> pathToLabeledHandling =
        recursiveHandlingPath(this.provider.getMergeSchemaForNamespaceURI(namespaceRef), scopeRef);
            ...
            if (comparator.compare(resultElement, patchElement) || foreignNamespace) {
              ...
              if (!foreignNamespace) {
                ...
                ElementMerger nestedMerger =
                    ElementMergerFactory.build(this.handlingScopeList, usedHandling, this.provider);
