...
public class XMLMergerTest {
  ...
  /**
   * Test method for {@link LeXeMerger#merge(Element, Element, ConflictHandlingType)} Tests if an Exception is thrown by
   ...
   *
   * @throws FileNotFoundException when the file is not present at the specified location
   * @throws ParserConfigurationException thrown by DOMHelper
   * @throws JAXBException when the file can't be parsed into a MergeSchema
   */
  ...
  /**
   * Tests {@link LeXeMerger#merge(Element, Element, ConflictHandlingType)} when the retrieved MergeSchema is non root
   *
  ...
  public void testNonRootMerge() throws Exception {
    ...
    Mockito.when(provider.getMergeSchemaForNamespaceURI(ArgumentMatchers.anyString())).thenReturn(schema);
  ...
  public void apiTest() throws Exception {
    ...
    String patchString = JDom2Util.getInstance().readFile("src/test/resources/systemtests/patches/FirstPatch.xml",
        charSet.name());
