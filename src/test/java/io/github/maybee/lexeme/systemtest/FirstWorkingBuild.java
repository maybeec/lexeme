package io.github.maybee.lexeme.systemtest;
...
public class FirstWorkingBuild {
    ...
    /**
     * Test for a simple use case. Used to ensure the basic implementation works as expected. Test case: First
     * .Expecting
     *
     * <pre>
     * &lt;A>
    ...
    public void FirstTest() throws Exception {
        String mergeSchemaLocation = resources + "mergeschemas/FirstMergeSchema.xml";

        Document base = JDom2Util.getInstance().getDocument(resources + "bases/FirstBase.xml");
        Document patch = JDom2Util.getInstance().getDocument(resources + "patches/FirstPatch.xml");
    ...
    public void SecondTest() throws Exception {
        ...
        Document patch = JDom2Util.getInstance().getDocument(resources + "patches/SecondPatch.xml");
        LeXeMerger testMerger = new LeXeMerger(mergeSchemaLocation);
        Element result =
            testMerger.merge(base.getRootElement(), patch.getRootElement(),
                ConflictHandlingType.PATCHATTACHOROVERWRITE);
    ...
    public void FirstBeanTest() throws Exception {
        ...
        Document base = JDom2Util.getInstance().getDocument(resources + "bases/Beans1.xml");
        Document patch = JDom2Util.getInstance().getDocument(resources + "patches/Beans1.xml");
        LeXeMerger testMerger = new LeXeMerger(mergeSchemaLocation);
        Element result =
            testMerger.merge(base.getRootElement(), patch.getRootElement(),
        ...
        assertEquals("Not all bean elements found", 3,
            result.getChildren("bean", Namespace.getNamespace("http://www.springframework.org/schema/beans"))
                .size());
        List<Element> beanNodeList =
            result.getChildren("bean", Namespace.getNamespace("http://www.springframework.org/schema/beans"));
        assertEquals(
            "Not all properties at bean1 found",
            2,
            beanNodeList
                .get(0)
                .getChildren("property",
                    Namespace.getNamespace("http://www.springframework.org/schema/beans")).size());
