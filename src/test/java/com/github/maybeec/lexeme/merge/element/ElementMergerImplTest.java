...
public class ElementMergerImplTest {
  ...
  /**
   * Test for <a href="https://github.com/maybeec/lexeme/issues/11">GitHub Issue #11<a/> <br/>
   * Based on {@link #testMergeElementWithChildElements()} but the A elements are marked as leafs Result should only
   * contain children from base document's A
   *
   * @throws Exception
   */
  @Test
  public void testLeafElementsBasePrefered() throws Exception {
    ...
    Element element1 = firstDoc.getRootElement();
    Element element2 = secondDoc.getRootElement();
    Element result = test.merge(element1, element2, ConflictHandlingType.BASEATTACHOROVERWRITE);

    assertTrue("No child for tag " + "B" + " found", result.getChildren("B").size() == 1);
    Element childResult = result.getChildren("B").get(0);

    assertEquals("Wrong text content: " + childResult.getText(), "abc", childResult.getText());

    assertTrue("Child for tag " + "C" + " found", result.getChildren("C").size() == 0);
   ...
   * Test for <a href="https://github.com/maybeec/lexeme/issues/11">GitHub Issue #11<a/> <br/>
   * Based on {@link #testMergeElementWithChildElements()} but the A elements are marked as leafs Result should only
   * contain children from patch document's A
   *
   * @throws Exception
   */
  @Test
  public void testLeafElementsPatchPrefered() throws Exception {
    ...
    assertEquals("Wrong text content: " + childResult.getText(), "def", childResult.getText());

    assertTrue("No child for tag " + "C" + " found", result.getChildren("C").size() == 1);
