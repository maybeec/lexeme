...
public class FullFunctionalityTest {
  ...
  public void ThirdBeanTest() throws Exception {
    ...
    String mergeSchemaLocation = this.resources + "mergeschemas";

    String bean = "http://www.springframework.org/schema/beans";
    String aop = "http://www.springframework.org/schema/aop";

    Document base = JDom2Util.getInstance().getDocument(this.resources + "bases/Beans3.xml");
    Document patch = JDom2Util.getInstance().getDocument(this.resources + "patches/Beans3.xml");
    ...
    assertEquals("Not as much bean as expected", result.getChildren("bean", Namespace.getNamespace(bean)).size(), 2);
    assertTrue("no aop:config found", result.getChildren("config", Namespace.getNamespace(aop)).size() == 1);
  ...
  public void ThirdSystemTest() throws Exception {
    ...
    Document base = JDom2Util.getInstance().getDocument(this.resources + "bases/ThirdBase.xml");
    Document patch = JDom2Util.getInstance().getDocument(this.resources + "bases/ThirdBase.xml");
    LeXeMerger testMerger = new LeXeMerger(mergeSchemaLocation);
